...
Action ComportamientoJugador::think(Sensores sensores) {
	// Capturar los valores de las filas y la columnas
...
struct nodo{
	...
	bool operator<(const nodo &n1) const{
		if(coste == n1.coste){
			return secuencia.size() > n1.secuencia.size();
		}

		else{
			return coste > n1.coste;
		}
	}
};
...
// Implementacion búsqueda por costo uniforme
// Entran los puntos origen y destino y devuelve la
// secuencia de acciones en plan, una lista de acciones.
bool ComportamientoJugador::pathFinding_CostoUniforme(const estado &origen, const estado &destino, list<Action> &plan) {
  ...
  nodo current;
	current.st = origen;
	current.secuencia.empty();
	current.coste = costeNodo(mapaResultado[current.st.fila][current.st.columna]);
  ...
  while (!abiertos.empty() and (current.st.fila!=destino.fila or current.st.columna != destino.columna)){
		...
		nodo hijoForward = current;
		if (!HayObstaculoDelante(hijoForward.st)){
			hijoForward.coste = costeNodo(mapaResultado[hijoForward.st.fila][hijoForward.st.columna]);
...
// Calcular el coste de un nodo
int ComportamientoJugador::costeNodo(char terreno){
	switch(terreno){
